...
//	Material parameters.
//----------------------------------------------------------------------
Real rho0_f = 1000;                      /**< Reference density of fluid. */
...
int main(int ac, char *av[])
{
    ...
    RegressionTestDynamicTimeWarping<ObservedQuantityRecording<Real>> write_recorded_water_pressure("Pressure", fluid_P_contact);
    BodyRegionByCell wave_probe_H1(water_block, makeShared<MultiPolygonShape>(createH1Shape(), "ObserverH1"));
    RegressionTestDynamicTimeWarping<ReducedQuantityRecording<UpperFrontInAxisDirection<BodyPartByCell>>>
        write_H1(wave_probe_H1, "FreeSurfaceHeight");
    BodyRegionByCell wave_probe_H2(water_block, makeShared<MultiPolygonShape>(createH2Shape(), "ObserverH2"));
    RegressionTestDynamicTimeWarping<ReducedQuantityRecording<UpperFrontInAxisDirection<BodyPartByCell>>>
        write_H2(wave_probe_H2, "FreeSurfaceHeight");
    BodyRegionByCell wave_probe_H3(water_block, makeShared<MultiPolygonShape>(createH3Shape(), "ObserverH3"));
    RegressionTestDynamicTimeWarping<ReducedQuantityRecording<UpperFrontInAxisDirection<BodyPartByCell>>>
        write_H3(wave_probe_H3, "FreeSurfaceHeight");
    //----------------------------------------------------------------------
    ...
    if (sph_system.RestartStep() != 0)
    {
        ...
        fluid_P_contact.updateConfiguration();
    }
    //----------------------------------------------------------------------
    ...
    write_H1.writeToFile(number_of_iterations);
    write_H2.writeToFile(number_of_iterations);
    write_H3.writeToFile(number_of_iterations);
    //----------------------------------------------------------------------
        ...
        while (integration_time < output_interval)
        {
            ...
            if (number_of_iterations % screen_output_interval == 0)
            {
                ...
                std::cout << std::fixed << std::setprecision(9) << "N=" << number_of_iterations << "	Time = "
                          ...
                          << "	advection_dt = " << advection_dt << "	acoustic_dt = " << acoustic_dt << " Remaining: " << hours << " h "
